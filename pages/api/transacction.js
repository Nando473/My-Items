...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      const result = await prisma.$transaction(async (prisma) => { // cambia parametros 
        try {
          const createdProduct = await prisma.product.create({  // en product cambia eso debido que es el nombre de tu tabla quequieres hacer la trancaccion 
            data: {
